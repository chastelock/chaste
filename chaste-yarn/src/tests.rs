...
macro_rules! test_workspace {
    ([$v:ident], $name:ident, $solver:expr) => {
        concat_idents!(fn_name = $v, _, $name {
            #[test]
            fn fn_name() -> Result<()> {
                ($solver)(parse(TEST_WORKSPACES.join(format!("{}_{}", stringify!($v), stringify!($name))))?, v_lv!($v), v_bun!($v))
            }
        });
    };
    ([$v:ident, $($vothers:ident),+], $name:ident, $solver:expr) => {
...
macro_rules! test_workspaces {
    ($name:ident, $solver:expr) => {
        #[cfg(feature = "classic")]
        test_workspace!([bun_v1, v1], $name, $solver);
        #[cfg(feature = "berry")]
        test_workspace!([v4, v6, v8], $name, $solver);
    };
}

test_workspaces!(basic, |chastefile: Chastefile, _lv: u8, _bun: bool| {
...
test_workspaces!(git_ssh, |chastefile: Chastefile, lv: u8, _bun: bool| {
    assert_eq!(
        chastefile
...
test_workspaces!(git_url, |chastefile: Chastefile, lv: u8, bun: bool| {
    assert_eq!(
        chastefile
            .recursive_package_dependencies(chastefile.root_package_id())
            .len(),
    ...
    if bun {
        // XXX: https://github.com/oven-sh/bun/issues/17091
        assert_eq!(minimatch.version(), None);
    } else {
        assert_eq!(minimatch.version().unwrap().to_string(), "10.0.1");
    }
    // berry: TODO: https://codeberg.org/selfisekai/chaste/issues/25
    // bun: XXX: https://github.com/oven-sh/bun/issues/17090
    assert_eq!(
...
test_workspaces!(github_ref, |chastefile: Chastefile, lv: u8, _bun: bool| {
    assert_eq!(
        chastefile
            .recursive_package_dependencies(chastefile.root_package_id())
            .len(),
...
#[cfg(feature = "classic")]
test_workspace!(
    // XXX: bun_v1 excluded due to https://github.com/oven-sh/bun/issues/17089
    [v1],
    npm_alias_duplicate,
    |chastefile: Chastefile, _lv: u8, _bun: bool| {
        assert_eq!(
            chastefile
                .recursive_package_dependencies(chastefile.root_package_id())
                .len(),
...
test_workspaces!(npm_aliased, |chastefile: Chastefile, lv: u8, _bun: bool| {
...
test_workspaces!(npm_tag, |chastefile: Chastefile, _lv: u8, _bun: bool| {
...
test_workspaces!(
    peer_unsatisfied,
    |chastefile: Chastefile, _lv: u8, bun: bool| {
        // XXX: https://github.com/oven-sh/bun/issues/16879
        if !bun {
...
test_workspaces!(resolutions, |chastefile: Chastefile, lv: u8, bun: bool| {
    ...
    // XXX: https://github.com/oven-sh/bun/issues/6608
    if !bun {
...
test_workspaces!(
    scope_registry,
    |chastefile: Chastefile, lv: u8, bun: bool| {
...
test_workspaces!(tarball_url, |chastefile: Chastefile, _lv: u8, bun: bool| {
    let empty_pid = chastefile.root_package_dependencies().first().unwrap().on;
    let empty_pkg = chastefile.package(empty_pid);
    assert_eq!(empty_pkg.name().unwrap(), "@a/empty");
    if bun {
...
test_workspaces!(
    workspace_basic,
    |chastefile: Chastefile, lv: u8, _bun: bool| {
